...
const dashboardUID = (0, core_1.getInput)('dashboardUID');
const grafanaToken = (0, core_1.getInput)('grafanaToken');
const mode = (0, core_1.getInput)('mode');
const release = (0, core_1.getInput)('release');
const rowList = (0, core_1.getInput)('rowList').split(',');
async function addAnnotationsGrafana(panelIds) {
    panelIds.forEach(async (panelId) => {
        ...
        const response = await (0, node_fetch_1.default)('http://localhost:3000/api/annotations', {
            method: 'POST',
            headers: {
...
async function listPanelsDashboardGrafana() {
    let panels = [];
    const response = await (0, node_fetch_1.default)(`http://localhost:3000/api/dashboards/uid/${dashboardUID}`, {
        headers: {
            'Authorization': `Bearer ${grafanaToken}`
        },
    });
    const { dashboard: { panels: elements } } = await response.json();
    let lastRowIndex = -1;
...
async function main() {
    let panelIds = await listPanelsDashboardGrafana();
